      ...
      <div v-if="modalFiltros" class="fixed inset-0 z-50 flex items-end bg-black/40 sm:items-center sm:justify-center" @click.self="modalFiltros = false">
        <div class="flex w-full flex-col rounded-t-xl bg-white sm:max-w-md sm:rounded-xl overflow-hidden shadow-2xl">
          ...
          <div class="flex items-center justify-between p-6 border-b border-gray-100">
            <div>
              <h3 class="text-xl font-extrabold text-gray-900">Filtrar Productos</h3>
              <p class="text-xs text-gray-500 font-medium">Gestión avanzada de inventario</p>
            </div>
          ...
          <div class="flex flex-col gap-6 p-6">
            <div class="space-y-2">
              <label class="text-xs font-bold text-gray-700 uppercase tracking-wider">Tipo de Producto</label>
            ...
            <div class="flex items-center gap-4 p-4 rounded-xl bg-primary/5 border border-primary/10">
              ...
              <label for="stockBajo" class="text-sm font-bold text-gray-800 cursor-pointer select-none">
                Solo stock bajo
                <span class="block text-[10px] text-gray-500 font-medium">Menos de 10 unidades en almacén</span>
              </label>
            </div>
          </div>
        ...
        <div class="flex w-full flex-col rounded-t-xl bg-white sm:max-w-md sm:rounded-xl">
          ...
          <div class="flex items-center justify-between p-4 border-b border-gray-200">
            <div class="flex items-center gap-3">
              ...
              <h3 class="text-lg font-bold text-text-primary">Eliminar Producto</h3>
            </div>
          ...
          <div class="flex flex-col gap-3 p-6 text-center">
            <p>¿Está seguro que desea eliminar <strong>{{ filaSeleccionada?.descripcion }}</strong>?</p>
          ...
          <div class="flex gap-3 p-4 border-t border-gray-200">
            ...
            <button @click="eliminarProducto" class="flex-1 rounded-lg bg-red-600 px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-red-700">
              <span class="material-symbols-outlined text-base">delete</span>
              Eliminar
            </button>
          </div>
...
async function eliminarProducto() {
  if (!filaSeleccionada.value) return
  await store.eliminarProducto(filaSeleccionada.value.id_producto)
