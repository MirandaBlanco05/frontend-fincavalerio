    ...
    <div class="w-full overflow-x-auto rounded-lg border border-border-color bg-white shadow-sm">
      <table class="w-full text-left text-sm text-text-primary">
        ...
        <tbody>
          ...
          <tr v-else v-for="cliente in store.clientes" :key="cliente.id_cliente" @click="seleccionarFila(cliente)" class="cursor-pointer border-b border-border-color bg-white transition hover:bg-primary/10" :class="{ 'bg-primary/20': filaSeleccionada?.id_cliente === cliente.id_cliente }">
            <td class="px-6 py-3 font-bold">#{{ cliente.id_cliente }}</td>
            <td class="px-6 py-3">{{ cliente.nombre }}</td>
            <td class="px-6 py-3">{{ cliente.rnc || '—' }}</td>
            <td class="px-6 py-3">{{ cliente.telefono }}</td>
            <td class="px-6 py-3">{{ cliente.correo }}</td>
            <td class="px-6 py-3">{{ cliente.provincia?.nombre || '—' }}</td>
            <td class="px-6 py-3">
              <span class="inline-block rounded-full px-2 py-0.5 text-xs font-bold" :class="cliente.estado === 'activo' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'">
                {{ cliente.estado }}
              </span>
            </td>
          </tr>
      ...
      <div v-if="modalAbierto" class="modal-overlay" @click.self="cerrarModal">
        <div class="modal-content">
          <div class="modal-header">
            <div>
              <h3 class="modal-title">{{ modoEdicion ? 'Editar' : 'Nuevo' }} Cliente</h3>
              <p class="modal-subtitle">Complete los datos del cliente</p>
            </div>
          ...
          <form @submit.prevent="guardar" class="modal-body">
            ...
            <div class="form-grid">
              <div class="form-group">
                ...
                <input v-model="form.rnc" type="text" class="form-input" placeholder="000-0000000-0" />
              </div>
      ...
      <div v-if="modalEliminar" class="modal-overlay" @click.self="modalEliminar = false">
        <div class="modal-content modal-content--small">
          ...
          <div class="modal-body">
            <p class="text-center">¿Está seguro que desea eliminar este cliente?</p>
          </div>
          <div class="modal-footer">
...
import { useClienteStore } from '@/modules/cliente/store/cliente.store.js'
...
function abrirModal(cliente = null) {
  if (cliente) {
    ...
    form.rnc = cliente.rnc || ''
    form.telefono = cliente.telefono
    form.correo = cliente.correo
    form.id_provincia = cliente.id_provincia || ''
...
async function guardar() {
  ...
  if (modoEdicion.value) {
    resultado = await store.actualizarCliente(filaSeleccionada.value.id_cliente, form)
  } else {
    resultado = await store.crearCliente(form)
  }
...
async function eliminar() {
  if (!filaSeleccionada.value) return
  const resultado = await store.eliminarCliente(filaSeleccionada.value.id_cliente)
  if (resultado.success) {
