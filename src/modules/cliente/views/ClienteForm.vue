    ...
    <div class="modal-content">
      ...
      <div class="modal-header">
        <div>
          ...
          <p class="modal-subtitle">Complete la información de contacto del cliente</p>
        </div>
        <button type="button" @click="router.push({ name: 'Clientes' })" class="btn-close">
      ...
      <form @submit.prevent="guardar" class="modal-body">
        ...
        <div class="form-group">
          ...
          <input v-model="form.nombre" type="text" required class="form-input" placeholder="Ej: Juan Pérez o Empresa S.A." />
        </div>
        ...
        <div class="form-group">
          ...
          <input v-model="form.correo" type="email" required class="form-input" placeholder="ejemplo@correo.com" />
        </div>
        ...
        <div class="modal-footer">
          <button type="button" @click="router.push({ name: 'Clientes' })" class="btn btn--secondary">Cancelar</button>
          <button type="submit" :disabled="store.cargando" class="btn btn--primary">
            <span class="material-symbols-outlined">save</span>
            {{ store.cargando ? 'Guardando...' : (modoEdicion ? 'Actualizar Cliente' : 'Guardar Cliente') }}
          </button>
        </div>
...
onMounted(async () => {
  ...
  if (modoEdicion.value) {
    ...
    const cli = store.clientes.find(c => c.id_cliente == route.params.id)
    if (cli) {
      form.nombre = cli.nombre
      form.rnc = cli.rnc
...
async function guardar() {
  let res
  if (modoEdicion.value) {
    res = await store.actualizarCliente(route.params.id, { ...form })
  } else {
    res = await store.crearCliente({ ...form })
  }
