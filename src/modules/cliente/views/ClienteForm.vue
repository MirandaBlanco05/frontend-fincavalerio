    ...
    <div class="modal-content">
      ...
      <form @submit.prevent="guardar" class="modal-body">
        ...
        <div class="modal-footer">
          <button type="button" @click="router.push({ name: 'Clientes' })" class="btn btn--secondary">
            Cancelar
          </button>
...
async function guardar() {
  ...
  try {
    let result
    if (modoEdicion.value) {
      result = await store.actualizarCliente(route.params.id, { ...form })
    } else {
      result = await store.crearCliente({ ...form })
    }

    if (result && result.success) {
      router.push({ name: 'Clientes' })
    } else {
      errorLocal.value = store.error || 'Error al guardar el cliente'
    }
  } catch (error) {
