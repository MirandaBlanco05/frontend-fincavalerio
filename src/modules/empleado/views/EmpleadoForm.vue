  ...
  <div class="modal-overlay">
    <div class="modal-content">
      ...
      <div class="modal-header">
        ...
        <button type="button" @click="router.push({ name: 'Empleados' })" class="btn-close">
      ...
      <form @submit.prevent="guardar" class="modal-body">
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input v-model="form.cedula" type="text" class="form-input" />
          </div>
        </div>
        ...
        <div class="form-grid">
          <div class="form-group">
            ...
            <select v-model="form.tipo_contrato" required class="form-select">
          ...
          <div class="form-group">
            ...
            <input v-model.number="form.salario" type="number" step="0.01" min="0" required class="form-input" />
          </div>
        </div>

        <div class="modal-footer">
          <button type="button" @click="router.push({ name: 'Empleados' })" class="btn btn--secondary">
            Cancelar
          </button>
...
onMounted(async () => {
  if (modoEdicion.value) {
    ...
    if (emp) {
      ...
      form.tipo_contrato = emp.tipo_contrato || 'Fijo'
      form.puesto = emp.puesto || ''
      form.salario = emp.salario || 0
    } else {
      errorLocal.value = 'No se pudo encontrar el empleado.'
    }
  }
})
...
async function guardar() {
  ...
  try {
    let result
    if (modoEdicion.value) {
      result = await store.actualizarEmpleado(route.params.id, { ...form })
    } else {
      result = await store.crearEmpleado({ ...form })
    }

    if (result && result.success) {
      router.push({ name: 'Empleados' })
    } else {
      errorLocal.value = store.error || 'Error al guardar el empleado'
    }
  } catch (error) {
