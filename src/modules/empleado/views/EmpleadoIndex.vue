      ...
      <div v-if="modalFiltros" class="fixed inset-0 z-50 flex items-end bg-black/40 sm:items-center sm:justify-center" @click.self="modalFiltros = false">
        <div class="flex w-full flex-col rounded-t-xl bg-white sm:max-w-md sm:rounded-xl overflow-hidden shadow-2xl">
          ...
          <div class="flex items-center justify-between p-6 border-b border-gray-100">
            <div>
              <h3 class="text-xl font-extrabold text-gray-900">Filtrar Personal</h3>
              <p class="text-xs text-gray-500 font-medium">Búsqueda avanzada de empleados</p>
            </div>
          ...
          <div class="flex flex-col gap-6 p-6">
            <div class="space-y-2">
              <label class="text-xs font-bold text-gray-700 uppercase tracking-wider">Nacionalidad</label>
              <select v-model="filtros.nacionalidad" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:border-primary focus:ring-4 focus:ring-primary/10 focus:outline-none transition-all bg-gray-50/50">
            ...
            <div class="space-y-2">
              <label class="text-xs font-bold text-gray-700 uppercase tracking-wider">Puesto / Cargo</label>
              <select v-model="filtros.puesto" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:border-primary focus:ring-4 focus:ring-primary/10 focus:outline-none transition-all bg-gray-50/50">
        ...
        <div class="flex w-full flex-col rounded-t-xl bg-white sm:max-w-md sm:rounded-xl">
          ...
          <div class="flex items-center justify-between p-4 border-b border-gray-200">
            <div class="flex items-center gap-3">
              ...
              <h3 class="text-lg font-bold text-text-primary">Eliminar Empleado</h3>
            </div>
          ...
          <div class="flex gap-3 p-4 border-t border-gray-200">
            ...
            <button @click="eliminarEmpleado" class="flex-1 rounded-lg bg-red-600 px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-red-700">
              <span class="material-symbols-outlined text-base">delete</span>
              Eliminar
            </button>
          </div>
...
async function eliminarEmpleado() {
  if (!filaSeleccionada.value) return
  await store.eliminarEmpleado(filaSeleccionada.value.id_empleado)
